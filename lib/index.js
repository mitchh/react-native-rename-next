...
var _replaceLast = require('replace-last');

var _replaceLast2 = _interopRequireDefault(_replaceLast);
...
readFile(_path2.default.join(__dirname, 'android/app/src/main/res/values/strings.xml')).then(function (data) {
  ...
  _commander2.default.version('2.5.3').arguments('<newName>').option('-b, --bundleID [value]', 'Set custom bundle identifier eg. "com.junedomingo.travelapp"').option('-B, --bundle-only', 'Do not require name change (newName must match existing)').action(function (newName) {
    ...
    if (_commander2.default.bundleOnly) {
      if (!_commander2.default.bundleID) {
    ...
    var resolveFoldersAndFiles = new Promise(function (resolve) {
      listOfFoldersAndFiles.forEach(function (element, index) {
        var dest = (0, _replaceLast2.default)(element, new RegExp(nS_CurrentAppName, 'i'), nS_NewName);
        ...
        setTimeout(function () {
          ...
          if (_fs2.default.existsSync(_path2.default.join(__dirname, element)) || !_fs2.default.existsSync(_path2.default.join(__dirname, element))) {
            if (element === dest && _commander2.default.bundleOnly) {
              // if bundle-only, this is not really a surprise.
              // console.log(`/${dest} ${colors.yellow('RENAME SKIPPED')}`);
            } else {
              var move = _shelljs2.default.exec('git mv "' + _path2.default.join(__dirname, element) + '" "' + _path2.default.join(__dirname, dest) + '"');
      ...
      return new Promise(function (resolve) {
        readFile(_path2.default.join(__dirname, 'android/app/src/main/AndroidManifest.xml')).then(function (data) {
          ...
          if (!_fs2.default.existsSync(fullNewBundlePath)) {
            ...
            var move = _shelljs2.default.exec('git mv "' + fullCurrentBundlePath + '/"* "' + fullNewBundlePath + '"');
    ...
    var resolveBundleIdentifiers = function resolveBundleIdentifiers(params) {
      return new Promise(function (resolve) {
        ...
        var newAndroidPath = _path2.default.join(__dirname, newBundlePath);
        _fs2.default.readdir(newAndroidPath, function (err, files) {
          //handling error
          if (err) {
            return console.log('Unable to scan directory: ' + err);
          }
          var filePaths = files.map(function (file) {
            return newAndroidPath + '/' + file;
          });
          (0, _nodeReplace2.default)(_extends({
